...
class DispatcherAgent(Dispatcher, GroupDispatcher):
    ...
    def _get_group_system_prompt(self, provider: str, num_cars: int, num_floors: int) -> str:
        ...
        if provider == "gemma":
            return (
                ...
                "- Cars have a weight limit (see current_weight_kg / max_weight_kg). A nearly-full car "
                "cannot pick up heavy waiting passengers — prefer dropping riders off first, and avoid "
                "sending a full car to answer hall calls it cannot serve.\n"
            ...
            return (
                ...
                "- Cars have a weight limit (current_weight_kg / max_weight_kg in tool output). A nearly-full "
                "  car cannot pick up heavy waiting passengers — prefer unloading before answering hall calls, "
                "  and route pickups to cars with enough remaining_weight_kg for the waiting passengers.\n"
                "- Ground your decisions strictly in the simulation state returned by the tools.\n"
    ...
    def dispatch(self, simulation: Any) -> int | None:
        ...
        system_prompt = (
            ...
            "corresponding to one of its onboard passengers' destinations so they can be delivered.\n"
            "- The car has a weight limit (current_weight_kg / max_weight_kg in tool output). When nearly "
            "full, prefer delivering riders before answering hall calls — heavy waiting passengers will be "
            "refused at the door if they don't fit.\n"
            "- Ground your decisions strictly in the simulation state returned by the tools.\n"
            "- You only have access to read-only state-gathering tools. Do NOT attempt to call "
