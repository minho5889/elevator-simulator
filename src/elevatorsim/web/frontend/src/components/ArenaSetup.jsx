...
export default function ArenaSetup({ onRace, racing }) {
    ...
    <div className="panel p-4 flex flex-col gap-4">
      ...
      <div>
        <div className="text-xs font-extrabold text-[var(--ink-2)] mb-2 uppercase tracking-wide">
          Contestants <span className="text-[var(--ink-3)] font-bold normal-case">({selected.length}/{MAX_CONTESTANTS})</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {LADDER.map((d) => {
            ...
            return (
              <button key={d.key} onClick={() => toggle(d.key)} title={d.blurb}
                ...
                {d.model && <span className="text-[9px] mono opacity-70">model</span>}
              </button>
            );
          })}
        </div>
      ...
      {/* Regime */}
      <div>
        <div className="text-xs font-extrabold text-[var(--ink-2)] mb-2 uppercase tracking-wide">Traffic</div>
        <div className="flex flex-wrap gap-2">
          {REGIMES.map((r) => {
            const on = config.regime === r.key;
            return (
              <button key={r.key} onClick={() => setCfg({ regime: r.key })} title={r.blurb}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3">
        <Slider label="Floors" value={config.num_floors} min={5} max={60} onChange={(v) => setCfg({ num_floors: v })} />
        <Slider label="Cars" value={config.num_cars} min={1} max={12} onChange={(v) => setCfg({ num_cars: v })} />
        <Slider label="Capacity" value={config.capacity} min={4} max={48} onChange={(v) => setCfg({ capacity: v })} />
        <Slider label="Arrival rate" value={config.arrival_rate} min={0.2} max={2.5} step={0.1} onChange={(v) => setCfg({ arrival_rate: v })} />
        <Slider label="Weight limit" value={config.max_weight_kg} min={300} max={3000} step={100} onChange={(v) => setCfg({ max_weight_kg: v })} suffix="kg" />
        <Slider label="Seed" value={config.seed} min={1} max={9999} onChange={(v) => setCfg({ seed: v })} />
      </div>
