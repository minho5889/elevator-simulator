...
- **G5 latency gate ✅ (2026-06-13)** — measured on real `gemma4:e4b`: 1.65s/decision, 100% valid (see §7).
- **P7 inference surface ✅ (2026-06-13, prep)** — `policy/structural_agent.py`: `LLMStructuralProvider` (direct `ollama.chat`, compact traffic-summary input, `think=False`, last-plan fallback, G4 valid-rate stats) + `make_structural_dispatcher`; frozen `STRUCTURAL_SYSTEM_PROMPT` in `policy/structural.py` (shared by inference and WO-003 assembly); arena `structural[:model]` rung. **End-to-end with the untuned base model** (real inference per epoch, 100% valid): matches the best fixed mode in up-peak (427 vs 420) and down-peak (426 vs 429), and is short in lunch/uniform (the 0-shot mode-classification gaps the oracle-distilled fine-tune will close). The fine-tuned `elevator-gemma` drops into this exact path via `OLLAMA_MODEL_ID` / `structural:elevator-gemma`.
- **Next: Lane B WO-002 (label driver, in flight) → WO-003 → Stage 4 LoRA → Stage 5 deploy (drop into the P7 path) ∥ P8 (optional double-deck).**
